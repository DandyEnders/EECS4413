...
public class View {
	...
	public String enrollmentAsHTML(SisModel model) throws Exception {
		...
		html = "<table border=\"1\"> <thead> <td>Course ID</td> <td>Student ID</td> <td>Course Credit</td> </thead>";
		Iterator iterator = hmap.entrySet().iterator();
		while (iterator.hasNext()) {
			Map.Entry me = (Map.Entry) iterator.next();
	...
	public String enrollmentAsHTML(SisModel model, Integer credit) throws Exception {
		String html = "";
		Map<String, EnrollmentBean> hmap = null;

		hmap = model.retriveEnrollment(credit);
